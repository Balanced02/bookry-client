...
const Alert = ({ alert, onDismiss }: { alert: AlertCreatedT, onDismiss: (key: string) => void }) => {
  ...
  React.useEffect(() => {
    let timeout: ReturnType<typeof setTimeout> = setTimeout(() => toggleVisibility(true), 100);
    let dismissTimeout: ReturnType<typeof setTimeout> = setTimeout(handleDismiss, 3000);
  ...
  return (
    <div className={`alert alert-${alert.type} ${visible ? 'visible' : ''}`}>
			...
			<div className="alert__body">
				...
				<div className="alert__description text-secondary">
					{alert.description}
				</div>			
			</div>
